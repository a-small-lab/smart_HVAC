...
ggplot(data = weather_energy %>% filter(totalEnergy > 25), aes(x=tempAvg, y=totalEnergy)) + geom_point() + geom_smooth()
```
...
# ETS
fit <- hierEnergy %>% fill_gaps() %>% fill(value, .direction = "down") %>%
  filter_index(~ "2021-03") %>%
