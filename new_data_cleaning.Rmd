...
# df$TimeThirty = strptime(df$TimeThirty, format ="%m/%d/%Y %H:%M")
...
AHU2E$tag = str_extract(AHU2E$Tagname, "(SAT$)|(SATSP)|(MAT)|(SFNSTS)|(SFNC)|(SFNVFDO)|(RFNVFD)|(RFNSTS)|(RFNC)|(SAF)|(SAFS)|(ZNT)")
...
AHU2W$tag = str_extract(AHU2W$Tagname, "(SAT$)|(SATSP)|(MAT)|(SFNSTS)|(SFNC)|(SFNVFDO)|(RFNVFD)|(RFNSTS)|(RFNC)|(SAF)|(SAFS)|(ZNT)")
...
pivot_wider(equipData, names_from = datapoint, values_from = value) -> equipUnstacked
...
# Calculate Enthalpy for Mixed and supply Air
```{r}
library(climateeng)
attach(equipUnstacked)
equipUnstacked$SAEnthalpy = enthalpy(SAT, 0.010) 
equipUnstacked$MAEnthalpy = enthalpy(MAT, 0.010)
equipUnstacked$EnthalpyChange = abs(SAEnthalpy - MAEnthalpy)
equipUnstacked$totalHeat = abs(SAEnthalpy - MAEnthalpy)*0.00047194745*SAF*1.202
